...
public class WapNavPageBarTag extends TagSupport {
	...
	public int doEndTag() throws JspException {
		try {
			...
			localJspWriter.write("<div style='margin:20px;text-align:center;'>");
	//		localJspWriter.write("<ul class='pagination'>");
			
			if (!("0".equals(this.v))) {
				if (this.e == 1){
				//	localJspWriter.write("<li class='page-pre disabled'><span>上一页</span></li>");// 上一页
					localJspWriter.write("<span class='pageBtn disabled' style='float:left;font-size:.7rem;opacity:.5;'>上一页</span>");
				}else{
				//	localJspWriter.write("<li><a href='javascript:pre_page(\"" + getId() + "\");'>上一页</a></li>");// 上一页
					localJspWriter.write("<span style='float:left'><a class='pageBtn' href='javascript:pre_page(\"" + getId() + "\");'>上一页</a></span>");
				}
				
				int pageNum  = 0;
				if(Integer.parseInt(this.v) % Integer.parseInt(this.u) != 0){
					pageNum = Integer.parseInt(this.v)/Integer.parseInt(this.u) + 1;
				}else{
					pageNum = Integer.parseInt(this.v)/Integer.parseInt(this.u);
				}
				
				localJspWriter.write("<span style='margin:0 auto;font-weight:bold;color:#666'>"+ this.e + " / "+ pageNum +"</span>");
				
			//	localJspWriter.write(getNumNavBar());
				if (this.f == this.e){
				//	localJspWriter.write("<li class='page-pre disabled'><span>下一页</span>");// 下一页
					localJspWriter.write("<span class='pageBtn disabled' style='float:right;font-size:.7rem;opacity:.5;'>下一页</span>");
				}else{
				//	localJspWriter.write("<li><a href='javascript:next_page(\"" + getId() + "\");'>下一页</a></li>");// 下一页
					localJspWriter.write("<span style='float:right'><a class='pageBtn' href='javascript:next_page(\"" + getId() + "\");'>下一页</a></span>");
				}
